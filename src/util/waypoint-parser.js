...
export async function parseKMZ(buf) {
  ...
  const templateXml = sharedDOMParser.parseFromString(templateStr, 'text/xml');
  const templateFolder = templateXml.querySelector('Document>Folder');
  const templateType = templateFolder.getElementsByTagName('wpml:templateType')[0].textContent;
  if (templateType === 'mapping2d') {
    templateFolder.querySelector('Placemark>Polygon coordinates')
      .textContent
      .replace(/[\n\t ]/g, '')
      .replace(/,0$/, '')
  ...
  const waylineXml = sharedDOMParser.parseFromString(waylineStr, 'text/xml');
  const waylinesFolders = Array.from(waylineXml.querySelectorAll('Document>Folder'));
  waylinesFolders.sort((a, b) => {
    const [ia, ib] = [a, b].map(e => Number.parseInt(e.getElementsByTagName('wpml:waylineId')[0].textContent, 10));
    return ia - ib;
  });
  for (const folder of waylinesFolders) {
